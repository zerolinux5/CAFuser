# ------------------------------------------------------------------------------
# Reference: https://github.com/timbroed/MUSES/blob/master/processing/radar_processing.py
# ------------------------------------------------------------------------------
...
from .utils import filter_and_project_pcd_to_image, motion_compensate_pcd, create_image_from_point_cloud, enlarge_points_in_image
...
def radar_to_point_cloud(front_radar_polar_image, intensity_threshold=0, image_fov_only=False, step=400,
                         ground_level=1.62, max_distance=150, resolution=0.0438):
    """
    ...
    """
    assert front_radar_polar_image.shape == (7547, step)
    if image_fov_only:
        min_azimuths = 150
        max_azimuths = 250
    else:
...
def load_radar_as_pcd(radar_path, intensity_threshold=0, image_fov_only=False, max_distance=150):
    """
...
def load_points_in_image_radar(radar_path,
    ...
    pcd_points = load_radar_as_pcd(radar_path, intensity_threshold, image_fov_only=True, max_distance=max_distance)
    ...
    uv_img_cords_filtered, pcd_points_filtered = filter_and_project_pcd_to_image(pcd_points, radar2rgb, K_rgb,
                                                                                 target_shape, max_distance=max_distance)
...
def load_radar_projection(radar_path, calib_data, scene_meta_dict=None, motion_compensation=False,
                          ...
                          dialtion_kernal=(10, 10), intensity_threshold=0, max_distance=150):
    """
    ...
    if enlarge_radar_points:
        image = enlarge_points_in_image(image, kernel_shape=dialtion_kernal)
    return image
