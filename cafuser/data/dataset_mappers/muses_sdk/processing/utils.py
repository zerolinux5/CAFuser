# ------------------------------------------------------------------------------
# Reference: https://github.com/timbroed/MUSES/blob/master/processing/utils.py
# ------------------------------------------------------------------------------
...
def enlarge_points_in_image(image, kernel_shape=(3, 3)):
    ...
    Args:
        ...
        kernel_shape (tuple or np.array): The shape of the kernel as a tuple (height, width) 
                                          or a numpy array for a custom kernel. Default is (3, 3).
    ...
    if isinstance(kernel_shape, np.ndarray):
        kernel = kernel_shape
    elif isinstance(kernel_shape, tuple):
        kernel = np.ones(kernel_shape, np.uint8)
    else:
        raise ValueError("kernel_shape must be a tuple or a numpy array.")
